...
:: Windows 11 Network Share Fix - CMD Menu Version
...
powershell -Command "$rp = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\LanmanWorkstation'; if (-not (Test-Path $rp)) { New-Item -Path $rp -Force ^| Out-Null }; Set-ItemProperty -Path $rp -Name 'AllowInsecureGuestAuth' -Value 1 -Type DWord -Force; Set-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\Lsa' -Name 'LmCompatibilityLevel' -Value 1 -Type DWord -Force -ErrorAction SilentlyContinue; Set-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Services\LanmanServer\Parameters' -Name 'SMB1' -Value 0 -Type DWord -Force -ErrorAction SilentlyContinue"
echo    [OK] Registry fixes applied.

echo.
echo    [5/5] Clearing Network Cache...
powershell -Command "ipconfig /flushdns ^| Out-Null; Restart-Service -Name 'LanmanWorkstation' -Force -ErrorAction SilentlyContinue; Restart-Service -Name 'LanmanServer' -Force -ErrorAction SilentlyContinue"
